...
public class Logic {
    ...
    private void createProcessND(Process process) throws ExcepcionDeAplicacion {
        ...
        try {
            ...
            cell4Source.add("\n");
            cell4Source.add("classesPaths=sorted(glob.glob(dataset+'/*'))\n");
            ...
            cell4Source.add("\n");
            cell4Source.add("os.makedirs(dataset+'/train')\n");
            cell4Source.add("os.makedirs(dataset+'/valid')\n");
            cell4Source.add("os.makedirs(dataset+'/images')\n");
            cell4Source.add("\n");
            cell4Source.add("for im in images:\n");
            cell4Source.add("  shutil.move(im,dataset+'/images/')\n");
            cell4Source.add("\n");
            cell4Source.add("for cl in classes:\n");
            cell4Source.add("  os.mkdir(dataset+'/train/'+cl)\n");
            cell4Source.add("  images=sorted(glob.glob(dataset+'/'+cl+'/*'))\n");
            cell4Source.add("  for i in range(int(len(images)*0.75)):\n");
            cell4Source.add("    images=sorted(glob.glob(dataset+'/'+cl+'/*'))\n");
            cell4Source.add("    j=random.randint(0,len(images)-1)\n");
            cell4Source.add("    shutil.move(images[j],dataset+'/train/'+cl)\n");
            cell4Source.add("  os.mkdir(dataset+'/valid/'+cl)\n");
            cell4Source.add("  images=sorted(glob.glob(dataset+'/'+cl+'/*'))\n");
            cell4Source.add("  for i in range(len(images)):\n");
            cell4Source.add("    shutil.move(images[i],dataset+'/valid/'+cl)\n");
            cell4Source.add("\n");
    ...
    private void createProcessIMD(Process process) throws ExcepcionDeAplicacion {
        FileWriter file = null;
        try {
            JSONObject root = new JSONObject();
            ...
            JSONObject metadata = new JSONObject();
            JSONObject colab = new JSONObject();
            colab.put("name", "Iterative Model Distillation.ipynb");
            colab.put("version", "0.3.2");
            colab.put("provenance", new JSONArray());
            ...
            JSONArray cell1Source = new JSONArray();
            cell1Source.add("## Iterative Model Distillation\n");
            cell1Source.add("\n");
            cell1.put("source", cell1Source);
            cells.add(cell1);
            ...
            cell4Source.add("shutil.copytree(dataset, 'dataset_IMD1')\n");
            cell4Source.add("omniModel('dataset_IMD1',listaModels," + process.getThreshold() + ")\n");
            
            cell4Source.add("listaModels2=[]\n");

            for (String model : process.getModels()) {
                cell4Source.add("learner2IMD_" + model.toLowerCase() + ",data=learn_with_model('dataset_IMD1',models." + model.toLowerCase() + ")\n");
                cell4Source.add("listaModels2.append(learner2IMD_" + model.toLowerCase() + ")\n");
            }
            ...
            file = new FileWriter("IterativeModelDistillation.ipynb");
            file.write(root.toJSONString());
            file.flush();
            file.close();
        } catch (IOException ex) {
    ...
    private void createProcessMDD(Process process) throws ExcepcionDeAplicacion {
        FileWriter file = null;
        try {
            JSONObject root = new JSONObject();
            ...
            JSONObject metadata = new JSONObject();
            JSONObject colab = new JSONObject();
            colab.put("name", "Model Data Distillation.ipynb");
            colab.put("version", "0.3.2");
            colab.put("provenance", new JSONArray());
            ...
            JSONArray cell1Source = new JSONArray();
            cell1Source.add("## Model Data Distillation\n");
            cell1Source.add("\n");
            cell1.put("source", cell1Source);
            cells.add(cell1);
            ...
            JSONArray cell2Source = new JSONArray();
            cell2Source.add("In this notebook we train models using model and data distillation.");
            cell2.put("source", cell2Source);
            cells.add(cell2);
            ...
            cell4Source.add("  return maximo, x[maximo]\n");
            cell4Source.add("\n");
            cell4Source.add("def omniModelData(dataset,learners,th):\n");
            cell4Source.add("  images=sorted(glob.glob(dataset+\"/images/*\"))\n");
            cell4Source.add("\n");
            cell4Source.add("  for image in images:\n");
            cell4Source.add("    lista=[]\n");
            ...
            file = new FileWriter("ModelDataDistillation.ipynb");
            file.write(root.toJSONString());
            file.flush();
            file.close();
        } catch (IOException ex) {
    ...
    private void createProcessIMDD(Process process) throws ExcepcionDeAplicacion{
        FileWriter file = null;
        try {
            JSONObject root = new JSONObject();
            ...
            JSONObject metadata = new JSONObject();
            JSONObject colab = new JSONObject();
            colab.put("name", "Iterative Model Data Distillation.ipynb");
            colab.put("version", "0.3.2");
            colab.put("provenance", new JSONArray());
            ...
            JSONArray cell1Source = new JSONArray();
            cell1Source.add("## Iterative Model Data Distillation\n");
            cell1Source.add("\n");
            cell1.put("source", cell1Source);
            cells.add(cell1);
            ...
            cell4Source.add("shutil.copytree(dataset, 'dataset_IMDD1')\n");
            cell4Source.add("omniModel('dataset_IMDD1',listaModels," + process.getThreshold() + ")\n");
            
            cell4Source.add("listaModels2=[]\n");
            for (String model : process.getModels()) {
                cell4Source.add("learner2IMDD_" + model.toLowerCase() + ",data=learn_with_model('dataset_IMDD1',models." + model.toLowerCase() + ")\n");
                cell4Source.add("listaModels2.append(learner2IMDD_" + model.toLowerCase() + ")\n");
            }
            ...
            file = new FileWriter("IterativeModelDataDistillation.ipynb");
            file.write(root.toJSONString());
            file.flush();
            file.close();
        } catch (IOException ex) {
